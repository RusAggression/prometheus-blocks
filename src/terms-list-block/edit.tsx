...
export function Edit() {
	...
	const groupedTerms = terms?.reduce((acc, term) => {
		const letter = term.title?.rendered?.charAt(0).toUpperCase() ?? '';
		acc[letter] ??= [];
	...
	return (
		<div {...blockProps}>
			{Object.keys(groupedTerms || {}).sort((a, b) => a.localeCompare(b)).map((letter) => (
						...
						{groupedTerms[letter].map((term) => (
							<Fragment key={term.id}>
								<dt className="terms__title">
									<a href={term.link} className="terms__link">
										{decodeEntities(term.title.rendered)}
									</a>
								</dt>
								<RichText.Content
									tagName="dd"
