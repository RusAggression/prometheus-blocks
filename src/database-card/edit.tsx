...
export function Edit({ context }: Readonly<BlockEditProps<{}>>): JSX.Element {
	...
	const featuredMedia = useSelect((select) => {
		...
		const coreSelect = select('core') as any;
		return coreSelect.getEntityRecord('postType', 'attachment', post.featured_media );
	}, [post?.featured_media]);

	const title = post?.title?.rendered ? decodeEntities(post.title.rendered) : '';
	...
	if (!postId) {
		return (
			<div {...blockProps}>
				<p>{ __('Database Card Block - Add this block inside a Post Template', 'database-cpt') }</p>
			</div>
		);
	}
		...
		<div {...blockProps}>
			{featuredMedia && (
				<div className="database-card__image">
					<a href={sourceLink} className="database-card__link" target="_blank" rel="noopener noreferrer">
						<img
